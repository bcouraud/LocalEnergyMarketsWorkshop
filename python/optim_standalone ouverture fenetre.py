...
TemperatureMin = np.array([15, 25.2, 21.1,20.3,21.64,24.6,24.5,21.6,21.3,21.1,20.2,5])
...
alphaThermal1_0 = alphaThermal1
alpha1_min = 0.005*1440/120*np.ones((1,LengthOptim))
alpha1_max = 0.02*1440/120*np.ones((1,LengthOptim))
...
Temperature_Ext = np.array([5, 5.2, 26.1,20.3,21.64,24.6,14.5,11.6,8.3,6.1,5.2,5])
...
# Vector x = [     real demand  ; available production ; demand needs       ; |sum of real demand  - sum of demand needs|;  |Demand needs - real demand| to assess flexibility cost;  Power heater;  Temperature ; alphaThermal] We take temperature as a constraint, not something to optimise. Could be changed.  

f =np.r_[BPrice,   np.zeros([LengthOptim]),  np.zeros([LengthOptim]),   EnergyDeficitCost  ,  flexCost*Priority,  BPrice,   np.zeros([LengthOptim]), np.zeros([LengthOptim])]
...
Aeq1 = np.c_ [ np.zeros((LengthOptim,LengthOptim)), np.eye(LengthOptim), np.zeros((LengthOptim,LengthOptim)), np.zeros([LengthOptim,1]), np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim))]
Aeq2 = np.c_ [ np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,LengthOptim)), np.eye(LengthOptim), np.zeros((LengthOptim,1)), np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim))]
Aeq3 = np.c_[ np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,1)), np.zeros((LengthOptim,LengthOptim)),-alphaThermal2*np.eye(LengthOptim),-(1-alphaThermal1_0)*(np.tri(LengthOptim,LengthOptim,-1))+np.eye(LengthOptim), np.diag(Temperature_Ext)]
...
Beq2=DemandRequired
Beq3=0*Temperature_Ext+(1-0)*np.r_[np.array([TemperatureInit]), np.zeros(LengthOptim-1)] 
Beq =np.concatenate([Beq1,Beq2,Beq3],axis=0)


A1 = np.c_[np.ones((1,LengthOptim)), np.zeros((1,LengthOptim)), -1*np.ones((1,LengthOptim)), np.array([0]),             np.zeros((1,LengthOptim)),  np.zeros((1,LengthOptim)), np.zeros((1,LengthOptim)), np.zeros((1,LengthOptim))]
...
A2 = np.c_[np.eye((LengthOptim)), -1*np.eye((LengthOptim)), np.zeros((LengthOptim,LengthOptim)), np.zeros(LengthOptim),             np.zeros((LengthOptim,LengthOptim)),  np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,LengthOptim))] #demand < production
A3 = np.c_[np.eye((LengthOptim)), np.zeros((LengthOptim,LengthOptim)), -1*np.eye((LengthOptim)), np.zeros(LengthOptim),           -1*np.eye((LengthOptim)),  np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,LengthOptim))] # |Demand needs - real demand| = X   (x >= (a-b) & x >= b-a)  zeros(LengthOptim)   zeros(LengthOptim)   zeros(LengthOptim)     zeros(LengthOptim,1) zeros(LengthOptim,1) -1*eye(LengthOptim)   %M(b-1) <= |Eb-Eb0| <= bM
A4 = np.c_[-1*np.eye((LengthOptim)), np.zeros((LengthOptim,LengthOptim)), np.eye((LengthOptim)), np.zeros(LengthOptim),           -1*np.eye((LengthOptim)),  np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,LengthOptim))] # |Demand needs - real demand| = X   To minimise |V-1|, where V is a positive continuous variable, just minimise x subject to x >= V-1 and x >= 1- V.

A5 = np.c_[np.ones((1,LengthOptim)), np.zeros((1,LengthOptim)), -1*np.ones((1,LengthOptim)), np.array([-1]),             np.zeros((1,LengthOptim)),  np.zeros((1,LengthOptim)), np.zeros((1,LengthOptim)), np.zeros((1,LengthOptim))]  #x2 =  |Daily ENergy Obtained - Daily ENergy needed |
A6 = np.c_[-1*np.ones((1,LengthOptim)), np.zeros((1,LengthOptim)), np.ones((1,LengthOptim)), np.array([-1]),             np.zeros((1,LengthOptim)),  np.zeros((1,LengthOptim)), np.zeros((1,LengthOptim)), np.zeros((1,LengthOptim))]  #x2 =  |Daily ENergy Obtained - Daily ENergy needed |
...
lb =np.r_[np.zeros((LengthOptim)),np.zeros((LengthOptim)), np.zeros((LengthOptim)), np.array([0]), np.zeros((LengthOptim)), np.zeros((LengthOptim)), TemperatureMin,alpha1_min[0]]
ub =np.r_[EmaxDaily*np.ones((LengthOptim)),EmaxDaily*np.ones((LengthOptim)), EmaxDaily*np.ones((LengthOptim)), np.array([EmaxDaily]), EmaxDaily*np.ones((LengthOptim)), Pmaxchauf*np.ones((LengthOptim)), TemperatureMax,alpha1_max[0]]
...
print(output)
output = ""
print("Alpha Convection: ")
for i in range(LengthOptim-1):
    output = output+str(solution[solution.shape[0]-1*LengthOptim+i])+","
print(output)
sys.stdout.flush()
# np.set_printoptions(linewidth=300)
# original_stdout = sys.stdout

x = np.linspace(0, LengthOptim*2,LengthOptim) 
# Plot the array
plt.plot(x,Temperature_Ext, label='Temp Ext')
plt.plot(x,TemperatureMin, label='Temp Min')
plt.plot(x,solution[solution.shape[0]-2*LengthOptim:solution.shape[0]-1*LengthOptim], label='Temp Int')
plt.plot(x,10*solution[solution.shape[0]-1*LengthOptim:solution.shape[0]], label='Alpha')
